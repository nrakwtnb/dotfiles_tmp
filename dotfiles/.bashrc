...
alias ptags='ctags -R $(find -name \*.py) -follow'
#alias ptags='ctags -R $(find -name \*.py)'# check difference from the above again
#export LANG=C.UTF-8
...
function docker_del() {
    containerID=${1:-abcdefgh}
    docker stop ${containerID} && docker rm ${containerID}
}

function docker_rm() {
    echo $(docker ps -aq --filter name=${1:-abcdefgh})
    docker rm $(docker ps -aq --filter name=${1:-abcdefgh})
}

function docker_rmi_since() {
    echo $(docker images -aq --filter "since=${1:-abcdefgh}")
    docker rmi $(docker images -aq --filter "since=${1:-abcdefgh}")
}

function docker_head() {
    numHead=${1:-5}
docker ps -n=${numHead} --no-trunc -s
}


function check_video() {
    ffprobe -select_streams v -show_streams $1
}
...
alias memory='watch -n 1 free -m'
alias jupy='nohup jupyter notebook &'
